...
def train_sess(sess, model_spec, num_steps, writer, params):
    ...
    # Adding F2 score to output
    precision_val = metrics_val['precision']
    recall_val = metrics_val['recall']
    f2_score = 5*precision_val*recall_val/(4*precision_val + recall_val)
    logging.info('- F2 Score:' + "{:05.3f}".format(f2_score))
...
def train_and_evaluate(train_model_spec, eval_model_spec, model_dir, params, restore_from=None):
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(begin_at_epoch, begin_at_epoch + params.num_epochs):
            ...
            eval_F2 = 5.0*precision*recall/(4.0*precision + recall)
            if eval_F2 >= best_eval_F2:
                # Store new best accuracy
                best_eval_F2 = eval_F2
                # Save weights
                best_save_path = os.path.join(model_dir, 'best_weights', 'after-epoch')
                best_save_path = best_saver.save(sess, best_save_path, global_step=epoch + 1)
                logging.info("- Found new best F2, saving in {}".format(best_save_path))
